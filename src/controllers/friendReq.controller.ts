...
export const sendFriendReq = async (req: Request, res: Response) => {
  ...
  const existingInvite = await FriendReq.findOne({ $or:[{ receiver, sender }, { receiver: sender, sender:receiver }] });
  if(existingInvite){
    const status = existingInvite.status;
    if(status==="pending"){
      return res.status(400).json({
        invite: "There is already an existing connection pending to be accepted/rejected"
      })
